    ...
    while 1:
        ...
        for i in range(len(queueGrid)):
            while (queueGrid[i].empty() is False):
                ...
                #if the myData is 3 long, and it passed both checks, valid communication
                if((len(myData)==3)):
                    passedsd = False
                    passedud = False
                    #check sd_ids for the sd_pid in myData
                    for j in range(sd_len):
                        if(myData[2]==sd_ids[j]):
                            passedsd = True
                            
                    #check ud_ids for the ud_pid in myData
                    for j in range(ud_len):
                        if(myData[1]==ud_ids[j]):
                            passedud = True
                    if (passedsd and passedud):        
                        print("A ping from",myData[2],"to",myData[1],"took",myData[0],"seconds") #print the data
                #print(myData)
                time.sleep(.3)
